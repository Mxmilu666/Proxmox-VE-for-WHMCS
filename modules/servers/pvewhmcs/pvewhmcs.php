...
function pvewhmcs_CreateAccount($params)
{
	...
	if (!empty($params['customfields']['KVMTemplate'])) {
		...
		if ($proxmox->login()) {
			...
			$vm_settings['name'] = "vps" . ($params["serviceid"] + 100) . "-cus" . $params['clientsdetails']['userid'];
			...
			if (strpos($response, 'UPID:') === 0) {
				...
				if (!empty($plan->ipv6) && $plan->ipv6 != '0') {
					...
					// 尝试使用interfaces接口获取IPv6地址
					$interfaces = $proxmox->get('/nodes/' . $first_node . '/qemu/' . ($params['serviceid'] + 100) . '/agent/get-interfaces');
						...
						foreach ($interfaces['result'] as $interface) {
							...
							if (isset($interface['ip-addresses']) && is_array($interface['ip-addresses'])) {
								foreach ($interface['ip-addresses'] as $addr) {
									// 检查是否为IPv6地址且非本地链接地址(非fe80开头)
									if (isset($addr['ip-address-type']) && $addr['ip-address-type'] == 'inet6' && 
										isset($addr['ip-address']) && strpos($addr['ip-address'], 'fe80::') !== 0) {
										$ipv6address = $addr['ip-address'];
										if (isset($addr['prefix'])) {
											$ipv6address .= '/' . $addr['prefix'];
										}
										break 2; // 找到全局IPv6地址后退出两层循环
									}
								}
							}
						}
					...
					// 如果通过agent无法获取，尝试其他方法
					if (empty($ipv6address)) {
						// 获取虚拟机状态
						$vm_status = $proxmox->get('/nodes/' . $first_node . '/qemu/' . ($params['serviceid'] + 100) . '/status/current');
						
						// 检查是否有IPv6地址
						if (isset($vm_status['net1']) && !empty($vm_status['net1'])) {
							// 提取IPv6地址
							$ipv6_info = $vm_status['net1'];
							if (preg_match('/ip6=([a-f0-9:\/]+)/i', $ipv6_info, $matches)) {
			...
			if ($proxmox->login()) {
				...
				if (strpos($response, 'UPID:') === 0) {
					...
					if (!empty($plan->ipv6) && $plan->ipv6 != '0') {
						...
						// 尝试使用interfaces接口获取IPv6地址
						$interfaces = $proxmox->get('/nodes/' . $first_node . '/lxc/' . ($params['serviceid'] + 100) . '/interfaces');
						
						if (isset($interfaces['data']) && is_array($interfaces['data'])) {
							// 遍历所有网络接口
							foreach ($interfaces['data'] as $interface) {
								// 跳过lo接口
								if ($interface['name'] === 'lo' || $interface['name'] === 'sit0') continue;
								...
								// 如果接口有inet6字段且非本地链接地址
								if (isset($interface['inet6']) && !empty($interface['inet6']) && strpos($interface['inet6'], 'fe80::') !== 0) {
									$ipv6address = $interface['inet6'];
									break;
								}
							}
						}
						
						// 如果通过interfaces无法获取，尝试其他方法
						if (empty($ipv6address)) {
							// 获取容器配置和状态
							$vm_config = $proxmox->get('/nodes/' . $first_node . '/lxc/' . ($params['serviceid'] + 100) . '/config');
							$vm_status = $proxmox->get('/nodes/' . $first_node . '/lxc/' . ($params['serviceid'] + 100) . '/status/current');
...
function pvewhmcs_SuspendAccount(array $params)
{
	...
	if ($proxmox->login()) {
		...
		$logrequest = '/nodes/' . $first_node . '/' . $guest->vtype . '/' . ($params['serviceid'] + 100) . '/status/stop';
		$response = $proxmox->post('/nodes/' . $first_node . '/' . $guest->vtype . '/' . ($params['serviceid'] + 100) . '/status/stop', $pve_cmdparam);
	}
	// DEBUG - Log the request parameters before it's fired
	if (Capsule::table('mod_pvewhmcs')->where('id', '1')->value('debug_mode') == 1) {
		logModuleCall(
...
function pvewhmcs_UnsuspendAccount(array $params)
{
	...
	if ($proxmox->login()) {
		...
		$pve_cmdparam = array();
		// Log and fire request
		$logrequest = '/nodes/' . $first_node . '/' . $guest->vtype . '/' . ($params['serviceid'] + 100) . '/status/start';
		$response = $proxmox->post('/nodes/' . $first_node . '/' . $guest->vtype . '/' . ($params['serviceid'] + 100) . '/status/start');
	}
	// DEBUG - Log the request parameters before it's fired
	if (Capsule::table('mod_pvewhmcs')->where('id', '1')->value('debug_mode') == 1) {
		logModuleCall(
...
function pvewhmcs_TerminateAccount(array $params)
{
	...
	if ($proxmox->login()) {
		...
		$guest_specific = $proxmox->get('/nodes/' . $first_node . '/' . $guest->vtype . '/' . ($params['serviceid'] + 100) . '/status/current');
		if ($guest_specific['status'] != 'stopped') {
			$proxmox->post('/nodes/' . $first_node . '/' . $guest->vtype . '/' . ($params['serviceid'] + 100) . '/status/stop', $pve_cmdparam);
			sleep(30);
		}

		if ($proxmox->delete('/nodes/' . $first_node . '/' . $guest->vtype . '/' . ($params['serviceid'] + 100), array('skiplock' => 1))) {
...
function pvewhmcs_ClientArea($params)
{
	...
	if ($proxmox->login()) {
		...
		foreach ($cluster_resources as $vm) {
			if ($vm['vmid'] == ($params['serviceid'] + 100) && $vm['type'] == $guest->vtype) {
				$vm_status = $vm;
				break;
			}
		}
		...
		if ($vm_status !== null) {
			...
			if ($guest->vtype == 'lxc') {
				...
				$ct_specific = $proxmox->get('/nodes/' . $first_node . '/lxc/' . ($params['serviceid'] + 100) . '/status/current');
	...
	logModuleCall('Proxmox VE', 'vm status', $vm_status, '', '', '');
...
function pvewhmcs_noVNC($params)
{
	...
	if ($proxmox->login()) {
		...
		$path = 'api2/json/nodes/' . $first_node . '/' . $guest->vtype . '/' . ($params['serviceid'] + 100) . '/vncwebsocket?port=' . $vm_vncproxy['port'] . '&vncticket=' . urlencode($vncticket);
...
function pvewhmcs_javaVNC($params)
{
	...
	if ($proxmox->login()) {
		...
		$guest = Capsule::table('mod_pvewhmcs_vms')->where('id', '=', $params['serviceid'])->get()[0];
		$vncparams = array();
		$vm_vncproxy = $proxmox->post('/nodes/' . $first_node . '/' . $guest->vtype . '/' . ($params['serviceid'] + 100) . '/vncproxy', $vncparams);
...
function pvewhmcs_vmStart($params)
{
	...
	if ($proxmox->login()) {
		...
		$pve_cmdparam = array();
		$logrequest = '/nodes/' . $first_node . '/' . $guest->vtype . '/' . ($params['serviceid'] + 100) . '/status/start';
		$response = $proxmox->post('/nodes/' . $first_node . '/' . $guest->vtype . '/' . ($params['serviceid'] + 100) . '/status/start', $pve_cmdparam);
	}
	// DEBUG - Log the request parameters before it's fired
	if (Capsule::table('mod_pvewhmcs')->where('id', '1')->value('debug_mode') == 1) {
		logModuleCall(
...
function pvewhmcs_vmReboot($params)
{
	...
	if ($proxmox->login()) {
		...
		if ($guest_specific['status'] = 'stopped') {
			...
			$response = $proxmox->post('/nodes/' . $first_node . '/' . $guest->vtype . '/' . ($params['serviceid'] + 100) . '/status/reboot', $pve_cmdparam);
		}
	}
	// DEBUG - Log the request parameters before it's fired
	if (Capsule::table('mod_pvewhmcs')->where('id', '1')->value('debug_mode') == 1) {
...
function pvewhmcs_vmShutdown($params)
{
	...
	if ($proxmox->login()) {
		...
		$logrequest = '/nodes/' . $first_node . '/' . $guest->vtype . '/' . ($params['serviceid'] + 100) . '/status/shutdown';
		$response = $proxmox->post('/nodes/' . $first_node . '/' . $guest->vtype . '/' . ($params['serviceid'] + 100) . '/status/shutdown', $pve_cmdparam);
	}
	// DEBUG - Log the request parameters before it's fired
	if (Capsule::table('mod_pvewhmcs')->where('id', '1')->value('debug_mode') == 1) {
		logModuleCall(
